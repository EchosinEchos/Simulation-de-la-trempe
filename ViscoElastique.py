...
for temps in range (1, Nt):
    ...
    for i in range(1,N):
        ...
        ajout_visco = - thorr[temps-1][i-1]*(1-dttho) + thorr[temps-1][i-2]*(1-dttho_prec) + 2*mu(i, temps)*err[temps-1][i-1] - 2*mu(i, temps)*err[temps-1][i-2]

        Rmat[2*(i-1)+2] = -3*k(i, temps)*alpha(i, temps)*DT[i-1]+3*k(i+1, temps)*alpha(i+1, temps)*DT[i-1] + ajout_visco
...
def Tcontraintesi(sol_ab,i,r, temps) -> ndarray:
    ...
    sigmai[0][0]= -4*mu(i, temps)*bi/(r**3) + 3*k(i, temps)*ai - 3*k(i, temps)*alpha(i, temps)*DT[i-1]
    sigmai[1][1]=  2*mu(i, temps)*bi/(r**3) + 3*k(i, temps)*ai - 3*k(i, temps)*alpha(i, temps)*DT[i-1]
    sigmai[2][2]=  2*mu(i, temps)*bi/(r**3) + 3*k(i, temps)*ai - 3*k(i, temps)*alpha(i, temps)*DT[i-1]
